...
!=== initialize with input/restart data or table values
  ...
  subroutine ConfigVarInTransfer(noahmp, NoahmpIO, N)
...
! ---------------------------------------------------------------------
    ...
    noahmp%config%nmlist%OptSurfaceStabilityFunction = NoahmpIO%PSI_OPT
    noahmp%config%nmlist%OptSurfaceThermalRoughness  = NoahmpIO%IZ0TLND
    ...
    noahmp%config%domain%SoilColor                   = NoahmpIO%SOILCOL(N)
    ...
    noahmp%config%domain%NumSnowLayerNeg             = NoahmpIO%ISNOWXY(N)
    noahmp%config%domain%NumSoilLayer                = NoahmpIO%NSOIL
	noahmp%config%domain%GridIndexI 			     = NoahmpIO%source_i(N)
	noahmp%config%domain%GridIndexJ 				 = NoahmpIO%source_j(N)
    noahmp%config%domain%TimeStepIndex               = NoahmpIO%ITIMESTEP
    ...
    noahmp%config%domain%GridSize                    = sqrt(max(10.0,NoahmpIO%DX(N)) * max(10.0,NoahmpIO%DY(N)))
    noahmp%config%domain%LandUseDataName             = NoahmpIO%LLANDUSE
    noahmp%config%domain%VegType                     = NoahmpIO%IVGTYP(N)
    noahmp%config%domain%CropType                    = NoahmpIO%CROPCAT(N)
    noahmp%config%domain%IndicatorIceSfc             = NoahmpIO%ICE(N)
    noahmp%config%domain%DayJulianInYear             = NoahmpIO%JULIAN
    noahmp%config%domain%NumDayInYear                = NoahmpIO%YEARLEN
    noahmp%config%domain%Latitude                    = NoahmpIO%XLAT(N)
    noahmp%config%domain%RefHeightAboveSfc           = NoahmpIO%FORCZLSM(N)
    noahmp%config%domain%ThicknessAtmosBotLayer      = NoahmpIO%DZ8W(NoahmpIO%KTS,N)
    noahmp%config%domain%CosSolarZenithAngle         = NoahmpIO%COSZEN(N) 
    ...
    noahmp%config%domain%RunoffSlopeType             = NoahmpIO%SLOPETYP(N)
    noahmp%config%domain%DepthSoilTempBottom         = NoahmpIO%ZBOT_TABLE
    noahmp%config%domain%FlagUseLocalPotTemp         = NoahmpIO%THSFC_LOC
    ...
    if ( noahmp%config%nmlist%OptSoilProperty == 1 ) then
       noahmp%config%domain%SoilType(1:NumSoilLayer) = NoahmpIO%ISLTYP(N)  ! soil type same in all layers
    elseif ( noahmp%config%nmlist%OptSoilProperty == 2 ) then
       noahmp%config%domain%SoilType(1) = nint(NoahmpIO%SOILCL1(N))        ! soil type in layer1
       noahmp%config%domain%SoilType(2) = nint(NoahmpIO%SOILCL2(N))        ! soil type in layer2
       noahmp%config%domain%SoilType(3) = nint(NoahmpIO%SOILCL3(N))        ! soil type in layer3
       noahmp%config%domain%SoilType(4) = nint(NoahmpIO%SOILCL4(N))        ! soil type in layer4
    elseif ( noahmp%config%nmlist%OptSoilProperty == 3 ) then
       noahmp%config%domain%SoilType(1:NumSoilLayer) = NoahmpIO%ISLTYP(N)  ! to initialize with default
    endif 
    ...
    if ( (NoahmpIO%IVGTYP(N) == NoahmpIO%ISURBAN_TABLE) .or. (NoahmpIO%IVGTYP(N) > NoahmpIO%URBTYPE_beg) ) then
       if ( NoahmpIO%SF_URBAN_PHYSICS == 0 ) then
           ...
           NoahmpIO%GVFMAX(N)         = 0.96                    ! unit: fraction
       endif
    endif

    ! treatment for crop point
    noahmp%config%domain%CropType = 0
    if ( (NoahmpIO%IOPT_CROP > 0) .and. (NoahmpIO%IVGTYP(N) == NoahmpIO%ISCROP_TABLE) ) &
       noahmp%config%domain%CropType = NoahmpIO%DEFAULT_CROP_TABLE   
       
    if ( (NoahmpIO%IOPT_CROP > 0) .and. (NoahmpIO%CROPCAT(N) > 0) ) then
       noahmp%config%domain%CropType = NoahmpIO%CROPCAT(N)
       noahmp%config%domain%VegType  = NoahmpIO%ISCROP_TABLE
       NoahmpIO%VEGFRA(N)          = 0.95                      ! unit: fraction
       NoahmpIO%GVFMAX(N)          = 0.95                      ! unit: fraction
    endif

    ! correct inconsistent soil type
    if ( any(noahmp%config%domain%SoilType == 14) .and. (NoahmpIO%XICE(N) == 0.0) ) then
