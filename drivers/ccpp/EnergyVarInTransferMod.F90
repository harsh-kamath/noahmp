...
!=== initialize with input data or table values
  ...
  subroutine EnergyVarInTransfer(noahmp, NoahmpIO, N)
...
! -------------------------------------------------------------------------
    ...
    noahmp%energy%state%LeafAreaIndex                             = NoahmpIO%LAI     (N)
    noahmp%energy%state%StemAreaIndex                             = NoahmpIO%XSAIXY  (N)
    noahmp%energy%state%SpecHumiditySfcMean                       = NoahmpIO%QSFC    (N)
    noahmp%energy%state%TemperatureGrd                            = NoahmpIO%TGXY    (N)
    noahmp%energy%state%TemperatureCanopy                         = NoahmpIO%TVXY    (N)
    noahmp%energy%state%SnowAgeNondim                             = NoahmpIO%TAUSSXY (N)
    noahmp%energy%state%AlbedoSnowPrev                            = NoahmpIO%ALBOLDXY(N)
    noahmp%energy%state%PressureVaporCanAir                       = NoahmpIO%EAHXY   (N)
    noahmp%energy%state%TemperatureCanopyAir                      = NoahmpIO%TAHXY   (N)
    noahmp%energy%state%ExchCoeffShSfc                            = NoahmpIO%CHXY    (N) 
    noahmp%energy%state%ExchCoeffMomSfc                           = NoahmpIO%CMXY    (N)
    noahmp%energy%state%TemperatureSoilSnow(-NumSnowLayerMax+1:0) = NoahmpIO%TSNOXY  (-NumSnowLayerMax+1:0,N)
    noahmp%energy%state%TemperatureSoilSnow(1:NumSoilLayer)       = NoahmpIO%TSLB    (1:NumSoilLayer,N)
    ...
    noahmp%energy%state%AlbedoSoilDir(1:NumSwRadBand)             = NoahmpIO%ALBSOILDIRXY(1:NumSwRadBand,N)
    noahmp%energy%state%AlbedoSoilDif(1:NumSwRadBand)             = NoahmpIO%ALBSOILDIFXY(1:NumSwRadBand,N)
    ...
    noahmp%energy%flux%HeatGroundTotAcc                            = NoahmpIO%ACC_SSOILXY(N)
    ...
    ! CCPP standard-name metadata supplies both fields as fractions in [0,1].
    noahmp%energy%param%VegFracAnnMax                              = NoahmpIO%GVFMAX(N)
    noahmp%energy%param%VegFracGreen                               = NoahmpIO%VEGFRA(N)
	! 
    ...
    if ( noahmp%config%nmlist%OptSoilProperty == 4 ) then
       noahmp%energy%param%SoilQuartzFrac(1:NumSoilLayer) = NoahmpIO%QUARTZ_3D(1:NumSoilLayer,N)
    endif
